...
namespace ChattingApp_Server
{
    ...
    public partial class MainWindow : Window
    {
                                    ...
                                    using (m_dbcon = new SQLiteConnection(dbConnection))
                                    {
                                        ...
                                        if (reader.HasRows)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                //{
                                                //    byte[] bufferInfo = Encoding.Default.GetBytes($"{user[0]}님이 채팅방에 입장하셨습니다.");
                                                //    Array.Resize(ref bufferInfo, bufferInfo.Length + 1);
                                                //    Array.Copy(bufferInfo, 0, bufferInfo, 1, bufferInfo.Length - 1);
                                                //    bufferInfo[0] = (int)State.Info;
                                                //    tempSocket.BeginSend(bufferInfo, 0, bufferInfo.Length, SocketFlags.None, new AsyncCallback(SendCallBack), string.Empty);
        ...
        private void test()
        {
            ...
            byte[] n = BitConverter.GetBytes(i);
            n = BitConverter.GetBytes(65535);
            n = BitConverter.GetBytes(2147483649);
            n = BitConverter.GetBytes(2147483647);
