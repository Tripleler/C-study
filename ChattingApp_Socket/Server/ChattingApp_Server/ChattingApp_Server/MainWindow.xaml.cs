...
namespace ChattingApp_Server
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void Receive_Completed(object sender, SocketAsyncEventArgs e)
        {
                    ...
                    try
                    {
                        ...
                        switch ((State)buffer[0])
                        {
                            case State.Login:
                                {
                                    ...
                                    using (m_dbcon = new SQLiteConnection(dbConnection))
                                    {
                                        ...
                                        {
                                            // 중복로그인 체크
                                            if (this.m_ClientSocket.Values.ToList().Contains(user[0]))
                                            {
                                                result[0] = (int)State.Login_Fail_duple;
                                                info = $"ID : {user[0]} 로그인 실패 : 현재 접속중인 아이디";
                                            }
                                            else
                                            {
                                                result[0] = (int)State.Login_Sucess;
                                                info = $"ID : {user[0]} 로그인 성공! 현재 접속중인 유저 수 : {this.m_ClientSocket.Count + 1}";
                                                ...
                                                bufferInfo[0] = (int)State.Info;
                                                for (int i = 0; i < sockets.Count; i++)
                                                {
                                                    Socket tempSocket = sockets[i];                                                    
                                                    tempSocket.BeginSend(bufferInfo, 0, bufferInfo.Length, SocketFlags.None, new AsyncCallback(SendCallBack), string.Empty);
                                                }
                                                this.m_ClientSocket.Add(ClientSocket, user[0]);
                                            }
                                        }
                                        else
                                        {
                                            result[0] = (int)State.Login_Fail_None;
                                            info = $"ID : {user[0]} 로그인 실패 : 결과값 없음";
                                        }
                                        ClientSocket.BeginSend(result, 0, result.Length, SocketFlags.None, new AsyncCallback(SendCallBack), info);
                                    }
                                    break;
                                }
                            case State.Register:
                                {
                                    ...
                                    using (m_dbcon = new SQLiteConnection(dbConnection))
                                    {
                                        ...
                                        {
                                            var reader = sqlCmd.ExecuteReader();
                                            result[0] = (int)State.Register_Sucess;
                                            info = $"ID : {user[0]} 회원가입 성공";
                                        }
                                        catch (Exception ex)
                                        {
                                            info = $"ID : {user[0]} 회원가입 실패 >> " + ex.Message;
                                            result[0] = (int)State.Register_Fail;
                                        }
                                        ClientSocket.BeginSend(result, 0, result.Length, SocketFlags.None, new AsyncCallback(SendCallBack), info);
                                    }
                                    break;
                            ...
                            case State.Message:
                                {
                                    if (this.m_ClientSocket.Count > 1)  // 접속중인 클라이언트가 둘 이상인 경우만
                                    {
                                        ...
                                        byte[] bufferInfo = new byte[1] { (int)State.Message };
                            ...
                            case State.File_Create:
                                {
                                    ...
                                    string guid = Encoding.Default.GetString(buffer, 5, 32);
                                    string fname = Encoding.Default.GetString(buffer, 37, buffer.Length - 37);
                                    string dir = SERVER_DIR + guid;
                                    FileStream fileStream = new FileStream(dir, FileMode.Create, FileAccess.Write, FileShare.Write);
                                    fileStream.Close();
                                    break;
                                }
                            case State.File_Append:
                                {
                                    string guid = Encoding.Default.GetString(buffer, 1, 32);
                                    byte[] bufferReceive = buffer.Skip(33).ToArray();
                                    string fname = Encoding.Default.GetString(buffer, 37, buffer.Length - 37);
                                    string dir = SERVER_DIR + guid;
                                    FileStream fileStream = new FileStream(dir, FileMode.Append, FileAccess.Write, FileShare.Write);
                                    BinaryWriter writer = new BinaryWriter(fileStream);
                                    try
                                    {
                                        writer.Write(bufferReceive, 0, bufferReceive.Length);
                                    }
                                    catch (Exception ex)
                                    {
                            ...
                            case State.File_End:
                                {
                                    string guid = Encoding.Default.GetString(buffer, 1, 32);
                                    string fname = Encoding.Default.GetString(buffer, 33, buffer.Length - 33);
                                    byte[] bufferInfo = new byte[1] { (int)State.file_info };
            ...
            else
            {
                try
                {
                    ...
                    byte[] buffer = new byte[1] { (int)State.Info };
        ...
        private void test()
        {
            //FileStream fileStream = new FileStream(SERVER_DIR + "check.txt", FileMode.Create, FileAccess.Write, FileShare.Write);
            //FileStream fileStream2 = new FileStream(SERVER_DIR + "check2.txt", FileMode.Create, FileAccess.Write, FileShare.Write);
            //BinaryWriter writer = new BinaryWriter(fileStream);
            //BinaryWriter writer2 = new BinaryWriter(fileStream2);
            //byte[] bufferReceive = Encoding.Default.GetBytes("가나다");
            //writer.Write(bufferReceive, 0, bufferReceive.Length);
            //bufferReceive = Encoding.Default.GetBytes("라마바");
            //writer.Write(bufferReceive, 0, bufferReceive.Length);
            //writer2.Write(bufferReceive, 0, bufferReceive.Length);
            //writer.Write(bufferReceive, 0, bufferReceive.Length);
            //writer.Close();
            //fileStream = new FileStream(SERVER_DIR + "닐리리야.txt", FileMode.Append, FileAccess.Write, FileShare.ReadWrite);
            //writer = new BinaryWriter(fileStream);
            //bufferReceive = Encoding.Default.GetBytes("자차카");
            //writer.Write(bufferReceive, 0, bufferReceive.Length);
            FileStream fileStream = new FileStream(SERVER_DIR + "check3.txt", FileMode.Create, FileAccess.Write, FileShare.Write);
            fileStream.Close();
        }
